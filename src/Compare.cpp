...
void Compare::runDijkstra(){
	...
	for(int i=0;i<240;i++)
		for(int j=0;j<240;j++)
		{
			dist.at<Vec3b>(j,i)={static_cast<unsigned char>(255-0.6*shortest_2d[i][j]), static_cast<unsigned char>(200-0.6*shortest_2d[i][j]), static_cast<unsigned char>(200-0.6*shortest_2d[i][j])};
		}
